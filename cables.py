...
height_cables = np.zeros(num_of_cables)
for i in range(num_of_cables):
	height_cables[i] = shortest_cable + ((i)/(num_of_cables-1))*(tallest_cable-shortest_cable) # height of secondary cables
...
Ecl = (vf/Ef+(1-vf)/Em)**(-1) # Perpicular
...
dia_cu_m = 2*np.sqrt(a_cu_m/pi) # [m] changing diameter of compsite cable upper for main cable
dia_cl_m = 2*np.sqrt(a_cl_m/pi) # [m] changing diameter of compsite cable lower for main cable

avg_dia_cu_m = np.mean(dia_cu_m) # [m] average diameter of compsite cable upper for secondary cable
avg_dia_cl_m = np.mean(dia_cl_m) # [m] average diameter of compsite cable lower for secondary cable
avg_dia_mix_m = avg_dia_cu_m+avg_dia_cl_m # [m] average diameter of compsite cable mix for secondary cable
...
dia_cu = 2*np.sqrt(a_cu/pi) # [m] changing diameter of compsite cable upper for secondary cable
dia_cl = 2*np.sqrt(a_cl/pi) # [m] changing diameter of compsite cable lower for secondary cable

avg_dia_cu = np.mean(dia_cu) # [m] average diameter of compsite cable upper for secondary cable
avg_dia_cl = np.mean(dia_cl) # [m] average diameter of compsite cable lower for secondary cable
avg_dia_mix = np.mean(avg_dia_cu+avg_dia_cl) # [m] average diameter of compsite cable mix for secondary cable
...
dia_bu_m = 2*np.sqrt(a_bu_m/pi) # [m] changing to diameter of main cables for bucky paper
avg_dia_bu_m = np.mean(dia_bu_m) # [m] average diameter of bucky paper for main cable


# Secondary cable 
a_bu = adj_force_rs/yield_b # [m**2] area of secondary cable for bucky paper
dia_bu = 2*np.sqrt(a_bu/pi) # [m] changing to diameter of secondary cables for bucky paper
avg_dia_bu = np.mean(dia_bu) # [m] average diameter of bucky paper for secondary cable

## Main Cables
print("Diameter of main cable on \n Mackinac bridge equals {} \n ".format(dia_rm))
print("Diameter of main cable made \n from Bucky paper equals {} \n ".format(avg_dia_bu_m))
print("Diameter of main cable with \n composite as transversely isotropic equals {} \n".format(avg_dia_cu_m))
print("Diameter of main cable with \n  composite as anisotropic equals {} \n".format(avg_dia_mix_m))

## Secondary Cables
print("Diameter of secondary cables on \n  cMackinac bridge equals {} \n".format(dia_rs))
print("Diameter of secondary cables made \n from Bucky paper equals {} \n".format(avg_dia_bu))
print("Diameter of secondary cable with \n composite as transversely isotropic equals {} \n ".format(avg_dia_cu))
print("Diameter of secondary cable with \n  composite as anisotropic equals {} \n".format(avg_dia_mix))
